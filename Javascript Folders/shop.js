...
function renderProducts(itemsToDisplay) {
    ...
    itemsToDisplay.forEach((product) => {
        ...
        if (product.isHair) {
            ...
            hairSelectHTML = `
                <div class="mt-2 mb-2">
                    <label class="block text-xs font-semibold mb-1">Select Length:</label>
                    <select onchange="updateHairPrice(event, ${product.id})" class="w-full text-xs p-1 rounded border border-gray-300 bg-white outline-none">
                        ${product.variants.map(v => `<option value="${v.price}">${v.length}</option>`).join('')}
                    </select>
                </div>
            `;
        }
        ...
        card.innerHTML = `
            <div>
                <div>
                    <img src="${product.img}" alt="${product.name}" class="rounded-xl w-full h-48 object-cover">
                </div>

                <div class="mt-2">
                    <h3 class="font-bold mb-1 text-base">${product.name}</h3>
                    <p class="text-xs mb-1 text-gray-800">${product.category}</p>
                    <p class="text-sm font-bold flex items-center gap-1">
                        <i class="fa-solid fa-naira-sign text-xs"></i>
                        <span id="price-${product.id}">${formatMoney(defaultPrice)}</span>
                    </p>
                </div>
            ...
            <div class="flex justify-center mt-3" id="action-area-${product.id}">
                <button 
        ...
        if (actionArea) {
            actionArea.innerHTML = `
                <div class="flex items-center justify-center gap-2 bg-white px-3 py-1 rounded-full border shadow-sm">
                    <button onclick="changeQty(${productId}, -1)" class="w-6 h-6 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center font-bold text-gray-700">-</button>
                    <span id="qty-${productId}" class="font-bold text-sm min-w-[18px] text-center">1</span>
                    <button onclick="changeQty(${productId}, 1)" class="w-6 h-6 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center font-bold text-gray-700">+</button>
...
function addToCart(productId) {
    const actionArea = document.getElementById(`action-area-${productId}`);
    const qtySpan = document.getElementById(`qty-${productId}`);
    const quantitySelected = qtySpan ? qtySpan.innerText : 1;
    ...
    if (actionArea) {
        actionArea.innerHTML = `
            <div class="bg-emerald-800 text-white text-xs font-bold px-6 py-1.5 rounded-full flex items-center justify-center gap-1.5 shadow-sm">
                ...
                <span>Added to Cart (${quantitySelected})</span>
            </div>
        `;
